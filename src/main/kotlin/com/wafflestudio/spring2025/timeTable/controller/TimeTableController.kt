...
class TimeTableController(
     ...
     */
    @PostMapping("/{timeTableId}/lectures") // 👈 이 API
    fun addLectureToTimeTable(
        @LoggedInUser user: User,
        @PathVariable("timeTableId") timeTableId: Long,
        @RequestBody request: AddLectureRequest, // 👈 DTO (예: data class AddLectureRequest(val lectureId: Long))
    ): ResponseEntity<TimeTableDetailResponse> {
        val updatedTimeTable = timeTableService.addLecture(user.id, timeTableId, request.lectureId)
